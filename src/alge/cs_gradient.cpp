...
inline static void
_fact_crout_pp(const int d_size, cs_real_t *ad)
{
  ...
  for (int kk = 0; kk < d_size - 1; kk++) {
    ...
    for (int ii = kk + 1; ii < d_size; ii++) {
      ...
      ad[ii_d_size + kk] = ad[ii_d_size + kk] / ad[kk_d_size + kk];
...
_gradient_info_create(const char *name, cs_gradient_type_t type)
{
...
static void
_gradient_info_update_iter(cs_gradient_info_t *this_info, int n_iter)
{
...
_gradient_info_dump(cs_gradient_info_t *this_info)
{
  ...
  cs_log_printf(CS_LOG_PERFORMANCE,
                _("\n"
                  ...
                  "  Number of calls:       %d\n"),
                this_info->name,
                cs_gradient_type_name[this_info->type],
                n_calls);
...
static cs_gradient_info_t *
_find_or_add_system(const char *name, cs_gradient_type_t type)
{
...
static double
_l2_norm_1(cs_lnum_t n_elts, cs_real_t *restrict x)
{
...
_get_clip_factor_try(const char *var_name)
{
  ...
  if (var_name != nullptr) {
    cs_field_t *f_c_iter =
      cs_field_by_composite_name_try("algo:grad_clip_factor", var_name);
    if (f_c_iter != nullptr)
      c_iter = f_c_iter->val;
  }
                          ...
                          cs_real_t (*restrict grad)[3])
{
  ...
  const cs_real_3_t *restrict cell_f_cen = (const cs_real_3_t *)fvq->cell_f_cen;
      ...
      for (int adj_id = 0; adj_id < n_adj; adj_id++) {
        ...
        for (cs_lnum_t i = 0; i < b_e_id; i++) { /* Loop on block elements */
          ...
          for (cs_lnum_t cidx = cell_cells_idx[c_id1];
            ...
            cs_real_t dist1 =
              std::abs(dist[0] * grad[c_id1][0] + dist[1] * grad[c_id1][1] +
                       dist[2] * grad[c_id1][2]);
...
#pragma omp parallel for if (n_cells > CS_THR_MIN)
      ...
      for (int adj_id = 0; adj_id < n_adj; adj_id++) {
        ...
        for (cs_lnum_t i = 0; i < b_e_id; i++) { /* Loop on block elements */
          ...
          for (cs_lnum_t cidx = cell_cells_idx[c_id1];
            ...
            cs_real_t dist1 =
              0.5 * std::abs(cs_math_3_dot_product(dist, gradm));
            cs_real_t dvar = std::abs(pvar[c_id1] - pvar[c_id2]);
  ...
  if (verbosity > 1) {
    ...
    cs_array_reduce_simple_stats_l(n_cells,
                                   1,
                                   nullptr,
                                   clip_factor,
                                   &min_factor,
      ...
      for (int t_id = 0; t_id < n_i_threads; t_id++) {
        for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
          ...
          if (c_weight_s != nullptr) {
            ...
            ktpond =
              weight[f_id] / 3.0 *
              (inv_sum[0] * c_weight_t[ii][0] + inv_sum[1] * c_weight_t[ii][1] +
        ...
        cs_real_t pfac =
          ...
          coefbp[f_id] *
            ((b_f_face_cog[f_id][0] - cell_f_cen[ii][0]) * f_ext[ii][0] +
             ...
             (b_f_face_cog[f_id][2] - cell_f_cen[ii][2]) * f_ext[ii][2] + poro);
...
#pragma omp parallel for
    ...
    if (cpl != nullptr)
      cs_internal_coupling_initialize_scalar_gradient(cpl,
                                                      c_weight,
                                                      pvar,
                                                      grad);
    ...
    for (int t_id = 0; t_id < n_b_threads; t_id++) {
      for (cs_lnum_t f_id = b_group_index[t_id * 2];
        ...
        */
        ...
        cs_real_t pfac = inc * coefap[f_id] + (coefbp[f_id] - 1.0) * pvar[ii];
                             ...
                             cs_real_3_t *restrict grad)
{
  ...
  const cs_real_3_t *restrict i_f_face_cog =
    (const cs_real_3_t *)fvq->i_f_face_cog;
  const cs_real_3_t *restrict b_face_cog = (const cs_real_3_t *)fvq->b_face_cog;
...
#pragma omp parallel for
    for (int t_id = 0; t_id < n_i_threads; t_id++) {
      for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
        ...
        for (cs_lnum_t i = 0; i < 3; i++) {
          for (cs_lnum_t j = 0; j < 3; j++) {
            cor_mat[ii][i][j] += (i_f_face_cog[f_id][i] - cell_f_cen[ii][i]) *
                                 i_f_face_normal[f_id][j];
            cor_mat[jj][i][j] -= (i_f_face_cog[f_id][i] - cell_f_cen[jj][i]) *
                                 i_f_face_normal[f_id][j];
          }
        }
      }
    ...
    for (cs_lnum_t face_id = b_group_index[t_id * 2];
      ...
      if (bc_type[face_id] == CS_OUTLET || bc_type[face_id] == CS_SYMMETRY) {
        ...
        for (int k = 0; k < 3; k++)
          for (int l = 0; l < 3; l++)
            cor_mat[ii][k][l] +=
              (xij[k] - xip[k]) * b_f_face_normal[face_id][l];
      }
    }
  }
                      ...
                      cs_gradient_quantities_t     *gq)
{
  ...
  const cs_real_3_t *restrict i_face_normal =
    (const cs_real_3_t *)fvq->i_f_face_normal;
  const cs_real_3_t *restrict dofij = (const cs_real_3_t *)fvq->dofij;
        ...
        for (int t_id = 0; t_id < n_i_threads; t_id++) {
          for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
            ...
            */
            ...
            /* Reconstruction part */
            cs_real_t dpfaci =
              pvar[c_id1] + cs_math_3_distance_dot_product(cell_f_cen[c_id1],
                                                           i_f_face_cog[f_id],
                                                           f_ext[c_id1]);

            cs_real_t dpfacj =
              pvar[c_id2] + cs_math_3_distance_dot_product(cell_f_cen[c_id2],
                                                           i_f_face_cog[f_id],
                                                           f_ext[c_id2]);

            cs_real_t pfaci =
              ktpond * poro[0] + (1.0 - ktpond) * poro[1] +
      ...
      for (int t_id = 0; t_id < n_b_threads; t_id++) {
        for (cs_lnum_t f_id = b_group_index[t_id * 2];
          ...
          */
          ...
          cs_real_t pfac =
            coefap[f_id] * inc +
            coefbp[f_id] * (cs_math_3_distance_dot_product(f_ext[c_id],
                                                           grad[c_id],
                                                           diipb[f_id]) +
                            poro);

          cs_real_t dpfac =
            pvar[c_id] + cs_math_3_distance_dot_product(cell_f_cen[c_id],
                                                        b_f_face_cog[f_id],
                                                        f_ext[c_id]);
...
#pragma omp parallel for
          ...
          for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
            ...
            /*
               Remark: \f$ \varia_\face = \alpha_\ij \varia_\celli
                                        + (1-\alpha_\ij) \varia_\cellj\f$
                       but for the cell \f$ \celli \f$ we remove
                       \f$ \varia_\celli \sum_\face \vect{S}_\face = \vect{0}
               \f$ and for the cell \f$ \cellj \f$ we remove \f$ \varia_\cellj
               \sum_\face \vect{S}_\face = \vect{0} \f$
            */
      ...
      /* Contribution from coupled faces */
      if (cpl != nullptr)
        cs_internal_coupling_iterative_scalar_gradient(cpl,
                                                       c_weight,
                                                       grad,
                                                       pvar,
                                                       rhs);
      ...
      for (int t_id = 0; t_id < n_b_threads; t_id++) {
        for (cs_lnum_t f_id = b_group_index[t_id * 2];
          ...
          */
          ...
          /* Reconstruction part */
          cs_real_t pfac = coefap[f_id] * inc +
                           coefbp[f_id] * (diipb[f_id][0] * grad[c_id][0] +
                            ...
                            cs_cocg_6_t                 *cocg)
{
  ...
  const cs_real_t *restrict b_face_surf = (const cs_real_t *)fvq->b_face_surf;
...
static void
_add_hb_faces_cell_cocg_lsq_orig(const cs_mesh_t             *m,
                                 const cs_mesh_quantities_t  *fvq,
                                 const cs_mesh_adjacencies_t *ma,
                                 cs_cocg_6_t                 *cocg)
{
...

{
  ...
  const cs_lnum_2_t *restrict i_face_cells =
    (const cs_lnum_2_t *)m->i_face_cells;
  const cs_lnum_t *restrict b_face_cells = (const cs_lnum_t *)m->b_face_cells;
  const cs_lnum_t *restrict b_cells      = (const cs_lnum_t *)m->b_cells;
  const cs_lnum_t *restrict cell_cells_idx =
  ...
  if (cocg == nullptr) {
    ...
    CS_MALLOC_HD(cocg, n_cells_ext, cs_cocg_6_t, CS_ALLOC_HOST_DEVICE_SHARED);
    CS_MALLOC_HD(cocgb, m->n_b_cells, cs_cocg_6_t, CS_ALLOC_HOST_DEVICE_SHARED);
  ...
  ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE(cs_lnum_t f_id) {
    ...
    cs_real_t ddc = 1.0 / (dc[0] * dc[0] + dc[1] * dc[1] + dc[2] * dc[2]);

    // Update cocg for cell ii
    cs_dispatch_sum<cs_real_t>(&cocg[ii][0],
                               dc[0] * dc[0] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][1],
                               dc[1] * dc[1] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][2],
                               dc[2] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][3],
                               dc[0] * dc[1] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][4],
                               dc[1] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[ii][5],
                               dc[0] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);

    // Update cocg for cell jj
    cs_dispatch_sum<cs_real_t>(&cocg[jj][0],
                               dc[0] * dc[0] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][1],
                               dc[1] * dc[1] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][2],
                               dc[2] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][3],
                               dc[0] * dc[1] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][4],
                               dc[1] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
    cs_dispatch_sum<cs_real_t>(&cocg[jj][5],
                               dc[0] * dc[2] * ddc,
                               CS_DISPATCH_SUM_ATOMIC);
  ...
  if (extended) {
    ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE(cs_lnum_t ii) {
      for (cs_lnum_t cidx = cell_cells_idx[ii]; cidx < cell_cells_idx[ii + 1];
           cidx++) {
        cs_real_t dc[3];
...
static void
_compute_cell_cocg_lsq_print(const cs_mesh_t            *m,
                             bool                        extended,
                             const cs_mesh_quantities_t *fvq,
                             cs_gradient_quantities_t   *gq)
{
...
#if defined(HAVE_ACCEL)
    CS_MALLOC_HD(cocg, n_cells_ext, cs_cocg_6_t, CS_ALLOC_HOST_DEVICE_SHARED);
    CS_MALLOC_HD(cocgb, n_b_cells, cs_cocg_6_t, CS_ALLOC_HOST_DEVICE_SHARED);
#else
    BFT_MALLOC(cocg, n_cells_ext, cs_cocg_6_t);
    BFT_MALLOC(cocgb, n_b_cells, cs_cocg_6_t);
#endif
  ...
  printf("After initialization loop:\n");
  for (cs_lnum_t c_id = 0; c_id < ((n_cells_ext < 5) ? n_cells_ext : 5);
       c_id++) {
    printf("cocg[%ld]:", c_id);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
      printf(" %f", cocg[c_id][ll]);
    }
    printf("\n");
  }
  ...
  /* Print after interior faces loop */
  printf("After interior faces loop:\n");
  for (cs_lnum_t c_id = 0; c_id < ((n_cells_ext < 5) ? n_cells_ext : 5);
       c_id++) {
    printf("cocg[%ld]:", c_id);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
  ...
  if (extended) {
    ...
    /* Print after extended neighborhood loop */
    printf("After extended neighborhood loop:\n");
    for (cs_lnum_t c_id = 0; c_id < ((n_cells < 5) ? n_cells : 5); c_id++) {
      printf("cocg[%ld]:", c_id);
      for (cs_lnum_t ll = 0; ll < 6; ll++) {
        printf(" %f", cocg[c_id][ll]);
      }
  ...
  /* Print after saving cocgb */
  printf("After saving cocgb:\n");
  for (cs_lnum_t ii = 0; ii < ((n_b_cells < 5) ? n_b_cells : 5); ii++) {
    printf("cocgb[%ld]:", ii);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
      printf(" %f", cocgb[ii][ll]);
    }
    printf("\n");
  }
  ...
  /* Print after boundary faces loop */
  printf("After boundary faces loop:\n");
  for (cs_lnum_t c_id = 0; c_id < ((n_cells_ext < 5) ? n_cells_ext : 5);
       c_id++) {
    printf("cocg[%ld]:", c_id);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
  ...
  /* Print after inversion loop */
  printf("After inversion loop:\n");
  for (cs_lnum_t c_id = 0; c_id < ((n_cells < 5) ? n_cells : 5); c_id++) {
    printf("cocg[%ld]:", c_id);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
      printf(" %f", cocg[c_id][ll]);
...
static void
_compute_cell_cocg_lsq_orig(const cs_mesh_t            *m,
                            bool                        extended,
                            const cs_mesh_quantities_t *fvq,
                            cs_gradient_quantities_t   *gq)
...
#pragma omp parallel for
  ...
  if (m->n_b_faces_all > m->n_b_faces)
    _add_hb_faces_cell_cocg_lsq_orig(m, fvq, cs_glob_mesh_adjacencies, cocg);
...
static void
_compute_cell_cocg_lsq_orig_print(const cs_mesh_t            *m,
                                  bool                        extended,
                                  const cs_mesh_quantities_t *fvq,
                                  cs_gradient_quantities_t   *gq)
{
...
#pragma omp parallel for
  ...
  /* Print after saving cocgb */
  printf("After saving cocgb:\n");
  for (cs_lnum_t ii = 0; ii < ((m->n_b_cells < 5) ? m->n_b_cells : 5); ii++) {
    printf("cocgb[%ld]:", ii);
    for (cs_lnum_t ll = 0; ll < 6; ll++) {
      printf(" %f", cocgb[ii][ll]);
    }
                   ...
                   cs_cocg_6_t *restrict      *cocgb)
{
  ...
  bool extended =
    (halo_type == CS_HALO_EXTENDED && m->cell_cells_idx) ? true : false;
...
static void
_get_cell_cocg_lsq_orig(const cs_mesh_t            *m,
                        cs_halo_type_t              halo_type,
                        bool                        accel,
                        const cs_mesh_quantities_t *fvq,
                        cs_cocg_6_t *restrict      *cocg,
                        cs_cocg_6_t *restrict      *cocgb)
{
  ...
  if (_cocg == nullptr)
    _compute_cell_cocg_lsq_orig(m, extended, fvq, gq);
                           ...
                           cs_cocg_t (*restrict cocg)[6])
{
  ...
  cs_lnum_t *cell_b_faces_idx         = (cs_lnum_t *)ma->cell_b_faces_idx;
  cs_lnum_t *cell_b_faces             = ma->cell_b_faces;

  cs_nreal_3_t *b_face_u_normal = fvq->b_face_u_normal;

  cs_real_t   *b_dist  = (cs_real_t *)fvq->b_dist;
  cs_real_3_t *diipb   = (cs_real_3_t *)fvq->diipb;
  cs_lnum_t   *b_cells = m->b_cells;
  /* Recompute cocg at boundaries, using saved cocgb */

  // Create dispatch context
  cs_dispatch_context ctx;
  cs_set_alloc_mode_r(b_cells, CS_ALLOC_HOST_DEVICE_SHARED);
  cs_set_alloc_mode_r(cell_b_faces_idx, CS_ALLOC_HOST_DEVICE_SHARED);
  cs_set_alloc_mode_r(cell_b_faces, CS_ALLOC_HOST_DEVICE_SHARED);
  cs_set_alloc_mode_r(coefbp, CS_ALLOC_HOST_DEVICE_SHARED);
  cs_set_alloc_mode_r(b_dist, CS_ALLOC_HOST_DEVICE_SHARED);
  cs_set_alloc_mode_r(b_face_u_normal, CS_ALLOC_HOST_DEVICE_SHARED);
  ...
  ctx.parallel_for(m->n_b_cells, [=] CS_F_HOST_DEVICE(cs_lnum_t ii) {
    ...
    for (cs_lnum_t i = s_id; i < e_id; i++) { /* loop on boundary faces */
      ...
      cs_real_t dddij[3];
      for (cs_lnum_t ll = 0; ll < 3; ll++)
        dddij[ll] = b_face_u_normal[f_id][ll] + umcbdd * diipb[f_id][ll];
...
static void
_recompute_lsq_scalar_cocg_orig(const cs_mesh_t            *m,
                                const cs_mesh_quantities_t *fvq,
                                const cs_field_bc_coeffs_t *bc_coeffs,
                                const cs_cocg_t (*restrict cocgb)[6],
                                cs_cocg_t (*restrict cocg)[6])
{
  ...
  const cs_lnum_t *restrict cell_b_faces = (const cs_lnum_t *)ma->cell_b_faces;
                     ...
                     cs_real_3_t *restrict grad)
{
  ...
  if (recompute_cocg)
    _recompute_lsq_scalar_cocg_orig(m, fvq, bc_coeffs, cocgb, cocg);
    ...
    for (int t_id = 0; t_id < n_i_threads; t_id++) {
      for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
        ...
        if (c_weight != nullptr) {
          ...
          cs_real_t denom =
            1. / (pond * c_weight[ii] + (1. - pond) * c_weight[jj]);
...
#pragma omp parallel for
  for (int t_id = 0; t_id < n_b_threads; t_id++) {
    for (cs_lnum_t f_id = b_group_index[t_id * 2];
      ...
      cs_real_t dsij[3];
      for (cs_lnum_t ll = 0; ll < 3; ll++)
        dsij[ll] = udbfs * b_face_normal[f_id][ll] + umcbdd * diipb[f_id][ll];

      cs_real_t pfac =
        (coefap[f_id] * inc + (coefbp[f_id] - 1.) * rhsv[ii][3]) * unddij;
                                  ...
                                  cs_real_3_t *restrict grad)
{
  ...
  if (recompute_cocg)
    _recompute_lsq_scalar_cocg(m, fvq, bc_coeffs, cocgb, cocg);
  ...
  // Create dispatch context
  cs_dispatch_context ctx;
  CS_MALLOC_HD(rhsv, n_cells_ext, cs_real_4_t, alloc_mode);
  if (is_porous == 0) {
    CS_MALLOC_HD(_f_ext_d, 3, cs_real_t, alloc_mode);
    // cs_arrays_set_value<cs_real_t, 1>(3, 0., _f_ext_d);
  ...
  if (e2n == CS_E2N_SUM_GATHER) {
    ...
    if (c_weight_s != nullptr) { /* With cell weighting */
      ...
      ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE(cs_lnum_t ii) {
        ...
        for(cs_lnum_t i = s_id; i < e_id; ++i) {
        //cs_sub_parallel_for(s_id, e_id, [=] (cs_lnum_t i) {
      ...
      ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE(cs_lnum_t ii) {
        ...
        const cs_lnum_t e_id = c2c_idx[ii + 1];
         ...
         //cs_sub_parallel_for(s_id, e_id, [=] (cs_lnum_t i) {
          ...
          cs_real_t dc[3] = {
                              dc[0] = cell_f_cen[jj][0] - cell_f_cen[ii][0],
                              dc[1] = cell_f_cen[jj][1] - cell_f_cen[ii][1],
                              dc[2] = cell_f_cen[jj][2] - cell_f_cen[ii][2]
                            };
  ...
  ctx.parallel_for(n_cells, [=] CS_F_HOST_DEVICE(cs_lnum_t c_id) {
    grad[c_id][0] = cocg[c_id][0] * rhsv[c_id][0] +
                    cocg[c_id][3] * rhsv[c_id][1] +
                    cocg[c_id][5] * rhsv[c_id][2] + f_ext[c_id][0];
    grad[c_id][1] = cocg[c_id][3] * rhsv[c_id][0] +
                    cocg[c_id][1] * rhsv[c_id][1] +
                    cocg[c_id][4] * rhsv[c_id][2] + f_ext[c_id][1];
    grad[c_id][2] = cocg[c_id][5] * rhsv[c_id][0] +
                    cocg[c_id][4] * rhsv[c_id][1] +
                    cocg[c_id][2] * rhsv[c_id][2] + f_ext[c_id][2];
  });

  /* Synchronize halos */
  ctx.wait();
  cs_sync_scalar_gradient_halo_d(m, CS_HALO_STANDARD, grad);
...
#pragma omp parallel for
      for (int t_id = 0; t_id < n_i_threads; t_id++) {
        cs_lnum_t s_id, e_id;
        cs_mesh_i_faces_thread_block_range(m,
                                           e2n,
                                           g_id,
                                           t_id,
                                           n_i_threads,
        ...
        if (c_weight_s != nullptr) { /* With cell weighting */
          ...
          for (cs_lnum_t f_id = s_id; f_id < e_id; f_id++) {
            ...
            pfac /= (pond * c_weight_s[ii] + (1. - pond) * c_weight_s[jj]);
                             ...
                             cs_real_3_t *restrict grad)
{
  ...
  bool warped_correction =
    (cs_glob_mesh_quantities_flag & CS_BAD_CELLS_WARPED_CORRECTION) ? true
                                                                    : false;
  ...
  // cs_real_3_t * v_grad = (cs_real_3_t *) grad;
  // cs_set_alloc_mode_r(v_grad, CS_ALLOC_HOST_DEVICE_SHARED);
  // grad = v_grad;
#else
  cs_real_4_t *restrict rhsv;
  BFT_MALLOC(rhsv, n_cells_ext, cs_real_4_t);
  ...
  if (hyd_p_flag == 1) {
    ...
    ctx.set_cuda_grid(0, 256);
    ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE(cs_lnum_t f_id) {
      // Local copies of face weight and face normal
      cs_real_t weight_f             = weight[f_id];
      cs_real_t i_f_face_normal_f[3] = { i_f_face_normal[f_id][0],
                                         i_f_face_normal[f_id][1],
                                         i_f_face_normal[f_id][2] };

      // Cell IDs for the two cells adjacent to the face
      cs_lnum_t c_id1 = i_face_cells[f_id][0];
      cs_lnum_t c_id2 = i_face_cells[f_id][1];

      // Local copies of cell variables
      cs_real_t c_var_c1 = c_var[c_id1];
      cs_real_t c_var_c2 = c_var[c_id2];

      // Compute ktpond with conditional weighting
      cs_real_t ktpond = weight_f; /* Default: no cell weighting */
      if (c_weight_s != nullptr) {
        cs_real_t c_weight_s_c1 = c_weight_s[c_id1];
        cs_real_t c_weight_s_c2 = c_weight_s[c_id2];
        cs_real_t denom =
          weight_f * c_weight_s_c1 + (1.0 - weight_f) * c_weight_s_c2;
        ktpond = (weight_f * c_weight_s_c1) / denom;
      }
      else if (c_weight_t != nullptr) {
        cs_real_t sum[6], inv_sum[6];
        cs_real_t c_weight_t_c1[6], c_weight_t_c2[6];

        // Load c_weight_t for both cells into local arrays
        for (int ii = 0; ii < 6; ii++) {
          c_weight_t_c1[ii] = c_weight_t[c_id1][ii];
          c_weight_t_c2[ii] = c_weight_t[c_id2][ii];
        }

        for (int ii = 0; ii < 6; ii++)
          sum[ii] =
            weight_f * c_weight_t_c1[ii] + (1.0 - weight_f) * c_weight_t_c2[ii];

        cs_math_sym_33_inv_cramer(sum, inv_sum);

        ktpond = (weight_f / 3.0) * (inv_sum[0] * c_weight_t_c1[0] +
                                     inv_sum[1] * c_weight_t_c1[1] +
                                     inv_sum[2] * c_weight_t_c1[2] +
                                     2.0 * (inv_sum[3] * c_weight_t_c1[3] +
                                            inv_sum[4] * c_weight_t_c1[4] +
                                            inv_sum[5] * c_weight_t_c1[5]));
      }

      // Porosity terms
      cs_real_t poro0 = i_poro_duq_0[is_porous * f_id];
      cs_real_t poro1 = i_poro_duq_1[is_porous * f_id];

      // Load face external forces and cell centers into local variables
      cs_real_t f_ext_c1[3]      = { f_ext[c_id1][0],
                                     f_ext[c_id1][1],
                                     f_ext[c_id1][2] };
      cs_real_t f_ext_c2[3]      = { f_ext[c_id2][0],
                                     f_ext[c_id2][1],
                                     f_ext[c_id2][2] };
      cs_real_t cell_f_cen_c1[3] = { cell_f_cen[c_id1][0],
                                     cell_f_cen[c_id1][1],
                                     cell_f_cen[c_id1][2] };
      cs_real_t cell_f_cen_c2[3] = { cell_f_cen[c_id2][0],
                                     cell_f_cen[c_id2][1],
                                     cell_f_cen[c_id2][2] };

      // Compute average external force
      cs_real_t fexd[3] = { 0.5 * (f_ext_c1[0] + f_ext_c2[0]),
                            0.5 * (f_ext_c1[1] + f_ext_c2[1]),
                            0.5 * (f_ext_c1[2] + f_ext_c2[2]) };

      // Load face center of gravity into local variable
      cs_real_t i_f_face_cog_f[3] = { i_f_face_cog[f_id][0],
                                      i_f_face_cog[f_id][1],
                                      i_f_face_cog[f_id][2] };

      // Compute deltas between face cog and cell centers
      cs_real_t delta_cog_c1[3] = { i_f_face_cog_f[0] - cell_f_cen_c1[0],
                                    i_f_face_cog_f[1] - cell_f_cen_c1[1],
                                    i_f_face_cog_f[2] - cell_f_cen_c1[2] };
      cs_real_t delta_cog_c2[3] = { i_f_face_cog_f[0] - cell_f_cen_c2[0],
                                    i_f_face_cog_f[1] - cell_f_cen_c2[1],
                                    i_f_face_cog_f[2] - cell_f_cen_c2[2] };

      // Compute pfaci and pfacj
      cs_real_t pfaci =
        ktpond *
          (delta_cog_c1[0] * f_ext_c1[0] + delta_cog_c1[1] * f_ext_c1[1] +
           delta_cog_c1[2] * f_ext_c1[2] + poro0) +
        (1.0 - ktpond) *
          (delta_cog_c2[0] * f_ext_c2[0] + delta_cog_c2[1] * f_ext_c2[1] +
           delta_cog_c2[2] * f_ext_c2[2] + poro1);

      cs_real_t pfacj = pfaci;

      // Compute variable differences
      cs_real_t c_var_diff = c_var_c2 - c_var_c1;

      pfaci += (1.0 - ktpond) * c_var_diff;
      pfacj -= ktpond * c_var_diff;

      /* Reconstruction part */
      cs_real_t delta_cen_c1[3] = { cell_f_cen_c1[0] - i_f_face_cog_f[0],
                                    cell_f_cen_c1[1] - i_f_face_cog_f[1],
                                    cell_f_cen_c1[2] - i_f_face_cog_f[2] };
      cs_real_t delta_cen_c2[3] = { cell_f_cen_c2[0] - i_f_face_cog_f[0],
                                    cell_f_cen_c2[1] - i_f_face_cog_f[1],
                                    cell_f_cen_c2[2] - i_f_face_cog_f[2] };

      cs_real_t rfac =
        weight_f * (delta_cen_c1[0] * fexd[0] + delta_cen_c1[1] * fexd[1] +
                    delta_cen_c1[2] * fexd[2]) +
        (1.0 - weight_f) *
          (delta_cen_c2[0] * fexd[0] + delta_cen_c2[1] * fexd[1] +
           delta_cen_c2[2] * fexd[2]);

      // Load dofij and r_grad values into local variables
      cs_real_t dofij_f[3] = { dofij[f_id][0], dofij[f_id][1], dofij[f_id][2] };
      cs_real_t r_grad_c1[3] = { r_grad[c_id1][0],
                                 r_grad[c_id1][1],
                                 r_grad[c_id1][2] };
      cs_real_t r_grad_c2[3] = { r_grad[c_id2][0],
                                 r_grad[c_id2][1],
                                 r_grad[c_id2][2] };

      rfac += 0.5 * (dofij_f[0] * (r_grad_c1[0] + r_grad_c2[0]) +
                     dofij_f[1] * (r_grad_c1[1] + r_grad_c2[1]) +
                     dofij_f[2] * (r_grad_c1[2] + r_grad_c2[2]));

      cs_real_t pfaci_plus_rfac = pfaci + rfac;
      cs_real_t pfacj_plus_rfac = pfacj + rfac;

      // Update gradients atomically
      for (int j = 0; j < 3; j++) {
        cs_real_t increment = pfaci_plus_rfac * i_f_face_normal_f[j];
        cs_real_t decrement = pfacj_plus_rfac * i_f_face_normal_f[j];

        cs_dispatch_sum<cs_real_t>(&grad[c_id1][j],
                                   increment,
                                   CS_DISPATCH_SUM_ATOMIC);
        cs_dispatch_sum<cs_real_t>(&grad[c_id2][j],
                                   -decrement,
                                   CS_DISPATCH_SUM_ATOMIC);
      }
    }); /* End of parallel_for_i_faces */
    ...
    ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE(cs_lnum_t f_id) {
      ...
      // Load coefficients
      cs_real_t coefap_f = coefap[f_id];
      cs_real_t coefbp_f = coefbp[f_id];

      // Load cell center and face center of gravity into local variables
      cs_real_t cell_f_cen_c[3]   = { cell_f_cen[c_id][0],
                                      cell_f_cen[c_id][1],
                                      cell_f_cen[c_id][2] };
      cs_real_t b_f_face_cog_f[3] = { b_f_face_cog[f_id][0],
                                      b_f_face_cog[f_id][1],
                                      b_f_face_cog[f_id][2] };

      // Compute delta between face cog and cell center
      cs_real_t delta[3] = { b_f_face_cog_f[0] - cell_f_cen_c[0],
                             b_f_face_cog_f[1] - cell_f_cen_c[1],
                             b_f_face_cog_f[2] - cell_f_cen_c[2] };

      // Load external force for the cell
      cs_real_t f_ext_c[3] = { f_ext[c_id][0], f_ext[c_id][1], f_ext[c_id][2] };

      // Compute dot product (delta . f_ext_c)
      cs_real_t dot_product =
        delta[0] * f_ext_c[0] + delta[1] * f_ext_c[1] + delta[2] * f_ext_c[2];

      // Compute pfac
      cs_real_t pfac = coefap_f * inc + coefbp_f * (dot_product + poro);

      // Load cell variable
      cs_real_t c_var_c = c_var[c_id];

      pfac += (coefbp_f - 1.0) * c_var_c;

      // Load diipb
      cs_real_t diipb_f[3] = { diipb[f_id][0], diipb[f_id][1], diipb[f_id][2] };

      // Load reconstructed gradient and compute difference with external force
      cs_real_t r_grad_c[3] = { r_grad[c_id][0],
                                r_grad[c_id][1],
                                r_grad[c_id][2] };

      cs_real_t r_grad_diff[3] = { r_grad_c[0] - f_ext_c[0],
                                   r_grad_c[1] - f_ext_c[1],
                                   r_grad_c[2] - f_ext_c[2] };

      // Compute rfac
      cs_real_t rfac =
        coefbp_f * (diipb_f[0] * r_grad_diff[0] + diipb_f[1] * r_grad_diff[1] +
                    diipb_f[2] * r_grad_diff[2]);

      // Load face normal
      cs_real_t b_f_face_normal_f[3] = { b_f_face_normal[f_id][0],
                                         b_f_face_normal[f_id][1],
                                         b_f_face_normal[f_id][2] };

      // Compute total factor
      cs_real_t total_factor = pfac + rfac;

      // Update gradient atomically
      cs_dispatch_sum<cs_real_t>(&grad[c_id][0], total_factor * b_f_face_normal_f[0], CS_DISPATCH_SUM_ATOMIC);
      cs_dispatch_sum<cs_real_t>(&grad[c_id][1], total_factor * b_f_face_normal_f[1], CS_DISPATCH_SUM_ATOMIC);
      cs_dispatch_sum<cs_real_t>(&grad[c_id][2], total_factor * b_f_face_normal_f[2], CS_DISPATCH_SUM_ATOMIC);
  ...
  /*---------------------------------------------*/
    ...
    ctx.parallel_for_i_faces(m, [=] CS_F_HOST_DEVICE(cs_lnum_t f_id) {
      ...
      */
      ...
      /* Reconstruction part */
      cs_real_t rfac =
        0.5 * (dofij[f_id][0] * (r_grad[c_id1][0] + r_grad[c_id2][0]) +
               dofij[f_id][1] * (r_grad[c_id1][1] + r_grad[c_id2][1]) +
               dofij[f_id][2] * (r_grad[c_id1][2] + r_grad[c_id2][2]));

      for (cs_lnum_t j = 0; j < 3; j++) {
        cs_dispatch_sum<cs_real_t>(&grad[c_id1][j],
                                   (pfaci + rfac) * i_f_face_normal[f_id][j],
                                   CS_DISPATCH_SUM_ATOMIC);
        cs_dispatch_sum<cs_real_t>(&grad[c_id2][j],
                                   -((pfacj + rfac) * i_f_face_normal[f_id][j]),
                                   CS_DISPATCH_SUM_ATOMIC);
      }
    }); /* End of parallel_for_i_faces */
    ...
    ctx.parallel_for_b_faces(m, [=] CS_F_HOST_DEVICE(cs_lnum_t f_id) {
      ...
      */
      ...
      cs_real_t pfac = inc * coefap[f_id] + (coefbp[f_id] - 1.0) * c_var[c_id];

      /* Reconstruction part */
      cs_real_t rfac = coefbp[f_id] * (diipb[f_id][0] * r_grad[c_id][0] +
                                       diipb[f_id][1] * r_grad[c_id][1] +
                                       diipb[f_id][2] * r_grad[c_id][2]);

      for (cs_lnum_t j = 0; j < 3; j++) {
        cs_dispatch_sum<cs_real_t>(&grad[c_id][j],
                                   (pfac + rfac) * b_f_face_normal[f_id][j],
                                   CS_DISPATCH_SUM_ATOMIC);
      }
    }); /* End of parallel_for_b_faces */
  ...
  /* Final adjustments on gradients */
    ...
    // Load cell_f_vol[c_id] into a local variable
    cs_real_t cell_f_vol_c = cell_f_vol[c_id];

    // Determine if the cell is disabled
    cs_lnum_t c_disable = 0;
    if (has_dc != 0) {
      c_disable = c_disable_flag[has_dc * c_id]; // Multiply by has_dc to handle the case when has_dc == 0
    }

    // Compute dvol based on whether the cell is disabled
    if (has_dc * c_disable == 0) {
      dvol = 1.0 / cell_f_vol_c;
    } else {
      dvol = 0.0;
    }

    // Load grad[c_id][0..2] into local variables
    cs_real_t grad_c[3];
    grad_c[0] = grad[c_id][0];
    grad_c[1] = grad[c_id][1];
    grad_c[2] = grad[c_id][2];

    // Multiply grad_c by dvol
    grad_c[0] *= dvol;
    grad_c[1] *= dvol;
    grad_c[2] *= dvol;

    if (warped_correction) {
      // Temporary storage for gradient correction
      cs_real_t gradpa[3];

      // Store grad_c into gradpa and reset grad_c
      gradpa[0] = grad_c[0]; grad_c[0] = 0.0;
      gradpa[1] = grad_c[1]; grad_c[1] = 0.0;
      gradpa[2] = grad_c[2]; grad_c[2] = 0.0;

      // Load corr_grad_lin[c_id] into local variables
      cs_real_t corr_grad_lin_c[3][3];
      corr_grad_lin_c[0][0] = corr_grad_lin[c_id][0][0];
      corr_grad_lin_c[0][1] = corr_grad_lin[c_id][0][1];
      corr_grad_lin_c[0][2] = corr_grad_lin[c_id][0][2];
      corr_grad_lin_c[1][0] = corr_grad_lin[c_id][1][0];
      corr_grad_lin_c[1][1] = corr_grad_lin[c_id][1][1];
      corr_grad_lin_c[1][2] = corr_grad_lin[c_id][1][2];
      corr_grad_lin_c[2][0] = corr_grad_lin[c_id][2][0];
      corr_grad_lin_c[2][1] = corr_grad_lin[c_id][2][1];
      corr_grad_lin_c[2][2] = corr_grad_lin[c_id][2][2];

      // Perform the matrix-vector multiplication
      grad_c[0] += corr_grad_lin_c[0][0] * gradpa[0]
                + corr_grad_lin_c[0][1] * gradpa[1]
                + corr_grad_lin_c[0][2] * gradpa[2];

      grad_c[1] += corr_grad_lin_c[1][0] * gradpa[0]
                + corr_grad_lin_c[1][1] * gradpa[1]
                + corr_grad_lin_c[1][2] * gradpa[2];

      grad_c[2] += corr_grad_lin_c[2][0] * gradpa[0]
                + corr_grad_lin_c[2][1] * gradpa[1]
                + corr_grad_lin_c[2][2] * gradpa[2];
    }

    // Write back the results to grad[c_id]
    grad[c_id][0] = grad_c[0];
    grad[c_id][1] = grad_c[1];
    grad[c_id][2] = grad_c[2];
  ...
  /* Synchronize halos */
  ctx.wait();
  cs_sync_scalar_gradient_halo_d(m, CS_HALO_EXTENDED, grad);
}

static void
_reconstruct_scalar_gradient_orig(const cs_mesh_t            *m,
                                  const cs_mesh_quantities_t *fvq,
                                  int                         w_stride,
                                  int                         hyd_p_flag,
                                  cs_real_t                   inc,
      ...
      for (int t_id = 0; t_id < n_i_threads; t_id++) {
        ...
        for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
          ...
          */
          ...
          cs_real_t pfaci =
            ktpond * ((i_f_face_cog[f_id][0] - cell_f_cen[c_id1][0]) *
                        f_ext[c_id1][0] +
                      (i_f_face_cog[f_id][1] - cell_f_cen[c_id1][1]) *
                        f_ext[c_id1][1] +
                      (i_f_face_cog[f_id][2] - cell_f_cen[c_id1][2]) *
                        f_ext[c_id1][2] +
                      poro[0]) +
            (1.0 - ktpond) * ((i_f_face_cog[f_id][0] - cell_f_cen[c_id2][0]) *
                                f_ext[c_id2][0] +
                              (i_f_face_cog[f_id][1] - cell_f_cen[c_id2][1]) *
                                f_ext[c_id2][1] +
                              (i_f_face_cog[f_id][2] - cell_f_cen[c_id2][2]) *
                                f_ext[c_id2][2] +
                              poro[1]);
    ...
    for (int t_id = 0; t_id < n_b_threads; t_id++) {
      for (cs_lnum_t f_id = b_group_index[t_id * 2];
        ...
        */
        ...
        /* Reconstruction part */
        cs_real_t rfac =
          coefbp[f_id] * (diipb[f_id][0] * (r_grad[c_id][0] - f_ext[c_id][0]) +
    ...
    for (int t_id = 0; t_id < n_b_threads; t_id++) {
      for (cs_lnum_t f_id = b_group_index[t_id * 2];
        ...
        */
        ...
        cs_real_t pfac =
          inc * coefap[f_id] + (coefbp[f_id] - 1.0) * c_var[c_id];
                       ...
                       cs_real_t *restrict b_f_var)
{
  ...
  const cs_field_bc_coeffs_t *_bc_coeffs =
    (bc_coeffs_loc != nullptr) ? (const cs_field_bc_coeffs_t *)bc_coeffs_loc
                               : (const cs_field_bc_coeffs_t *)bc_coeffs;
...
#pragma omp parallel for if (n_b_cells > CS_THR_MIN)
  for (cs_lnum_t ci = 0; ci < n_b_cells; ci++) {
    ...
    for (cs_lnum_t i = s_id; i < e_id; i++) {
      ...
      cs_real_t pip = c_var[c_id] + cs_math_3_dot_product(diipb[f_id], grad);
                            ...
                            cs_real_t *restrict b_f_var)
{
  ...
  const cs_lnum_t *restrict cell_b_faces_idx =
    (const cs_lnum_t *)ma->cell_b_faces_idx;
  const cs_lnum_t *restrict cell_cells   = (const cs_lnum_t *)ma->cell_cells;
  const cs_lnum_t *restrict cell_cells_e = (const cs_lnum_t *)ma->cell_cells_e;
  const cs_lnum_t *restrict cell_b_faces = (const cs_lnum_t *)ma->cell_b_faces;
...
#pragma omp parallel for if (n_b_cells > CS_THR_MIN)
  for (cs_lnum_t ci = 0; ci < n_b_cells; ci++) {
    ...
    for (int adj_id = 0; adj_id < n_adj; adj_id++) {
      ...
      if (c_weight == nullptr) {
        for (cs_lnum_t i = s_id; i < e_id; i++) {
          ...
          cs_real_t pfac =
            (c_var[c_id1] - c_var[c_id] - 0.5 * dc[0] * f_ext[c_id][0] -
             0.5 * dc[1] * f_ext[c_id][1] - 0.5 * dc[2] * f_ext[c_id][2] -
             0.5 * dc[0] * f_ext[c_id1][0] - 0.5 * dc[1] * f_ext[c_id1][1] -
             0.5 * dc[2] * f_ext[c_id1][2]) *
            ddc;
    ...
    for (cs_lnum_t i = s_id; i < e_id; i++) {
      ...
      /* (b_f_face_cog - cell_f_cen).f_ext, or IF.F_i */
      cs_real_t c_f_ext = cs_math_3_distance_dot_product(cell_f_cen[c_id],
                                                         b_f_face_cog[f_id],
                                                         f_ext[c_id]);

      cs_real_t pfac =
        (bc_coeff_a[f_id] * inc +
         (bc_coeff_b[f_id] - 1.) * (c_var[c_id] + c_f_ext + poro)) *
        unddij;
    ...
    grad[0] = (a11 * rhsv[0] + a12 * rhsv[1] + a13 * rhsv[2]) * det_inv +
              f_ext[c_id][0];
    grad[1] = (a12 * rhsv[0] + a22 * rhsv[1] + a23 * rhsv[2]) * det_inv +
              f_ext[c_id][1];
    grad[2] = (a13 * rhsv[0] + a23 * rhsv[1] + a33 * rhsv[2]) * det_inv +
              f_ext[c_id][2];
...
{
  cs_real_t retval = t[0] * t[0] + t[1] * t[1] + t[2] * t[2] + 2 * t[3] * t[3] +
                     2 * t[4] * t[4] + 2 * t[5] * t[5];
  return retval;
}
...
{
  const cs_real_3_t *restrict coefav  = (const cs_real_3_t *)bc_coeffs_v->a;
  const cs_real_33_t *restrict coefbv = (const cs_real_33_t *)bc_coeffs_v->b;
...
#pragma omp parallel for
    for (int t_id = 0; t_id < n_i_threads; t_id++) {
      for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
        ...
        cs_real_t ktpond =
          (c_weight == nullptr) ? pond : // no cell weighting
            pond * c_weight[c_id1]       // cell weighting active
              / (pond * c_weight[c_id1] + (1.0 - pond) * c_weight[c_id2]);
...
#pragma omp parallel for
  ...
  cs_lnum_t n_i_groups, n_i_threads;
  cs_mesh_i_faces_thread_block_count(m,
                                     CS_E2N_SUM_SCATTER,
                                     0,
                                     &n_i_groups,
                                     &n_i_threads);
      ...
      for (cs_lnum_t f_id = s_id; f_id < e_id; f_id++) {
        ...
        */
        ...
        for (cs_lnum_t i = 0; i < stride; i++) {
          ...
          cs_real_t rfac =
            0.5 *
            (dofij[f_id][0] * (r_grad[c_id1][i][0] + r_grad[c_id2][i][0]) +
             dofij[f_id][1] * (r_grad[c_id1][i][1] + r_grad[c_id2][i][1]) +
             dofij[f_id][2] * (r_grad[c_id1][i][2] + r_grad[c_id2][i][2]));
...
#pragma omp parallel for if (n_b_cells > CS_THR_MIN)
  for (cs_lnum_t ii = 0; ii < n_b_cells; ii++) {
    ...
    for (cs_lnum_t fidx = s_id; fidx < e_id;
         fidx++) { /* loop on boundary faces */
...
#pragma omp parallel for
  ...
  if (cs_glob_timer_kernels_flag > 0) {
    ...
    elapsed =
      std::chrono::duration_cast<std::chrono::microseconds>(t_init - t_start);
    printf(", init = %ld", elapsed.count());

    elapsed =
      std::chrono::duration_cast<std::chrono::microseconds>(t_i_faces - t_init);
    printf(", i_faces = %ld", elapsed.count());

    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t_b_faces -
                                                                    t_i_faces);
    printf(", b_faces = %ld", elapsed.count());

    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t_rescale -
                                                                    t_b_faces);
    printf(", rescale = %ld", elapsed.count());

    elapsed =
      std::chrono::duration_cast<std::chrono::microseconds>(t_stop - t_rescale);
                           ...
                           cs_real_33_t *restrict grad)
{
  ...
  if (l2_norm > cs_math_epzero) {
    ...
    for (isweep = 1; isweep < n_r_sweeps && l2_residual > epsrgp * l2_norm;
         isweep++) {
...
#pragma omp parallel for
          ...
          for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
            ...
            */
            ...
            for (cs_lnum_t i = 0; i < 3; i++) {
              ...
              cs_real_t pfaci =
                0.5 *
                ((grad[c_id1][i][0] + grad[c_id2][i][0]) * dofij[f_id][0] +
                 (grad[c_id1][i][1] + grad[c_id2][i][1]) * dofij[f_id][1] +
                 (grad[c_id1][i][2] + grad[c_id2][i][2]) * dofij[f_id][2]);
              cs_real_t pfacj = pfaci;
      ...
      /* Contribution from coupled faces */
      if (cpl != nullptr)
        cs_internal_coupling_iterative_vector_gradient(cpl,
                                                       c_weight,
                                                       grad,
                                                       pvar,
                                                       rhs);
...
{
  const cs_real_6_t *restrict coefat  = (const cs_real_6_t *)bc_coeffs_ts->a;
  const cs_real_66_t *restrict coefbt = (const cs_real_66_t *)bc_coeffs_ts->b;
  ...
  for (cs_lnum_t i = s_id; i < e_id; i++) {
    ...
    for (int ll = 0; ll < 9; ll++) {
      ...
      for (int pp = 0; pp <= ll; pp++) {
        ...
        cocgb_v[ll_9 + pp] -=
          (nb[ss] * bt[rr][kk] * iipbf[qq] + nb[qq] * bt[kk][rr] * iipbf[ss]) *
          db;
      }
    }
    ...
    for (int ll = 0; ll < 9; ll++) {
      ...
      for (int rr = 0; rr < 3; rr++) {
        rhsv += bt[rr][pp] * diipb[f_id][qq] *
                (a[rr] + bt[rr][0] * pvar[c_id][0] + bt[rr][1] * pvar[c_id][1] +
                 bt[rr][2] * pvar[c_id][2]);
      }
    ...
    for (int t_id = 0; t_id < n_i_threads; t_id++) {
      for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
        ...
        if (c_weight != nullptr) {
          cs_real_t pond = weight[f_id];
          cs_real_t denom =
            1. / (pond * c_weight[c_id1] + (1. - pond) * c_weight[c_id2]);
      ...
      for (cs_lnum_t i = 0; i < 3; i++) {
        cs_real_t pfac = coefav[f_id][i] * inc +
                         (coefbv[f_id][0][i] * pvar[c_id1][0] +
                          ...
                          coefbv[f_id][2][i] * pvar[c_id1][2] - pvar[c_id1][i]);
...
#pragma omp parallel
  {
    ...
    for (cs_lnum_t b_c_id = t_s_id; b_c_id < t_e_id; b_c_id++) {
      ...
      _fw_and_bw_ldtl_pp(cocgb_v, 9, x, rhsb_v);
...
_get_c_iter_try(const char *var_name)
{
  ...
  if (var_name != nullptr) {
    cs_field_t *f_c_iter =
      cs_field_by_composite_name_try("algo:grad_b_iter", var_name);
    if (f_c_iter != nullptr)
      c_iter = f_c_iter->val;
  }
 ...
 *   b_iter_count   --> iteration count for each face, or nullptr
 *(postprocessing) grad           --> gradient of pvar (du_i/dx_j :
 *grad[][i][j])
 *----------------------------------------------------------------------------*/
                      ...
                      cs_real_t (*restrict grad)[stride][3])
{
  ...
  const cs_real_3_t *restrict cell_cen = (const cs_real_3_t *)fvq->cell_f_cen;
  const cs_real_t *restrict weight     = fvq->weight;
...
#pragma omp parallel for schedule(dynamic, CS_THR_MIN)
  for (cs_lnum_t c_idx = 0; c_idx < n_b_cells; c_idx++) {
    ...
    for (n_c_it = 0; n_c_it < n_c_iter_max; n_c_it++) {
      ...
      for (cs_lnum_t i = s_id; i < e_id; i++) {
        ...
        const cs_real_t *b =
          ((const cs_real_t *)coefbv) + (f_id * stride * stride);
      ...
      for (cs_lnum_t ii = 0; ii < stride; ii++) {
        grad_c[ii][0] =
          (cocg[c_id][0] * rhs_c[ii][0] + cocg[c_id][3] * rhs_c[ii][1] +
           cocg[c_id][5] * rhs_c[ii][2]);
        grad_c[ii][1] =
          (cocg[c_id][3] * rhs_c[ii][0] + cocg[c_id][1] * rhs_c[ii][1] +
           cocg[c_id][4] * rhs_c[ii][2]);
        grad_c[ii][2] =
          (cocg[c_id][5] * rhs_c[ii][0] + cocg[c_id][4] * rhs_c[ii][1] +
  ...
  if (cs_glob_timer_kernels_flag > 0) {
    ...
    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t_ext_n -
                                                                    t_i_faces);
    ...
    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(t_gradient -
                                                                    t_b_faces);
    printf(", gradient = %ld", elapsed.count());

    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
      t_b_correction - t_gradient);
    printf(", b_correction = %ld", elapsed.count());

    elapsed = std::chrono::duration_cast<std::chrono::microseconds>(
      t_halo - t_b_correction);
    printf(", halo = %ld", elapsed.count());
 ...
 *   c_weight       <-- weighted gradient coefficient variable, or nullptr
 *   b_iter_count   --> iteration count for each face, or nullptr
 *(postprocessing) gradv          --> gradient of pvar (du_i/dx_j : gradv[][j])
 *----------------------------------------------------------------------------*/
                        ...
                        cs_real_t (*restrict b_f_var)[stride])
{
  ...
  const cs_field_bc_coeffs_t *_bc_coeffs =
    (bc_coeffs_v_loc != nullptr) ? (const cs_field_bc_coeffs_t *)bc_coeffs_v_loc
                                 : (const cs_field_bc_coeffs_t *)bc_coeffs_v;
...
#pragma omp parallel for if (n_b_faces > CS_THR_MIN)
  for (cs_lnum_t f_id = 0; f_id < n_b_faces; f_id++) {
    ...
    for (cs_lnum_t k = 0; k < stride; k++) {
      b_f_var[f_id][k] =
        inc * bc_coeff_a[f_id][k] +
        (bc_coeff_b[f_id][0][k] * pip[0] + bc_coeff_b[f_id][1][k] * pip[1] +
...
#pragma omp parallel
  {
    ...
    for (cs_lnum_t b_c_id = t_s_id; b_c_id < t_e_id; b_c_id++) {
      ...
      _fw_and_bw_ldtl_pp(cocgb_t, 18, x, rhsb_t);
                 ...
                 cs_real_t (*restrict grad)[3])
{
  ...
  if (cpl != nullptr && gradient_type != CS_GRADIENT_GREEN_ITER) {
    if (bc_coeffs_loc == nullptr) {
  ...
  switch (gradient_type) {
    ...
    case CS_GRADIENT_GREEN_R:
      ...
      _renormalize_scalar_gradient(mesh, fvq, hyd_p_flag, grad);
    ...
    case CS_GRADIENT_GREEN_LSQ: {
...
#if defined(HAVE_CUDA)
        if (cs_get_device_id() > -1 && hyd_p_flag <= 1)
          amode = CS_ALLOC_DEVICE;
          // else if (cs_get_device_id() > -1 && hyd_p_flag == 1)
      ...
      if (w_stride == 6 && c_weight != nullptr)
        ...
        if (cs_get_device_id() > -1)
          _lsq_scalar_gradient_hyd_p_gather<CS_E2N_SUM_GATHER>(mesh,
                                                               fvq,
                                                               halo_type,
                                                               recompute_cocg,
                                                               inc,
                                                               ...
                                                               c_weight,
                                                               r_grad);
        else if (cs_glob_e2n_sum_type == CS_E2N_SUM_SCATTER)
          _lsq_scalar_gradient_hyd_p<CS_E2N_SUM_SCATTER>(mesh,
                                                         fvq,
                                                         halo_type,
                                                         recompute_cocg,
                                                         inc,
          ...
          _lsq_scalar_gradient_hyd_p<CS_E2N_SUM_SCATTER_ATOMIC>(mesh,
                                                                fvq,
                                                                halo_type,
                                                                recompute_cocg,
                                                                inc,
          ...
          _lsq_scalar_gradient_hyd_p<CS_E2N_SUM_GATHER>(mesh,
                                                        fvq,
                                                        halo_type,
                                                        recompute_cocg,
                                                        inc,
          ...
          _lsq_scalar_gradient_hyd_p<_e2n_sum_type>(mesh,
                                                    fvq,
                                                    halo_type,
                                                    recompute_cocg,
                                                    inc,
                       ...
                       cs_real_t                   c_grad[stride][3])
{
  ...
  for (cs_lnum_t i = 0; i < stride; i++) {
    c_grad[i][0] =
      rhs[i][0] * cocg[0] + rhs[i][1] * cocg[3] + rhs[i][2] * cocg[5];

    c_grad[i][1] =
      rhs[i][0] * cocg[3] + rhs[i][1] * cocg[1] + rhs[i][2] * cocg[4];

    c_grad[i][2] =
      rhs[i][0] * cocg[5] + rhs[i][1] * cocg[4] + rhs[i][2] * cocg[2];
  }
  ...
  if (e_id > s_id && bc_coeff_b != nullptr) {
    ...
    for (n_c_it = 0; n_c_it < n_c_iter_max; n_c_it++) {
      ...
      for (cs_lnum_t i = s_id; i < e_id; i++) {
        ...
        const cs_real_t *b =
          ((const cs_real_t *)bc_coeff_b) + (f_id * stride * stride);
      ...
      for (cs_lnum_t ii = 0; ii < stride; ii++) {
        grad_c[ii][0] = (cocg[0] * rhs_c[ii][0] + cocg[3] * rhs_c[ii][1] +
                         cocg[5] * rhs_c[ii][2]);
        grad_c[ii][1] = (cocg[3] * rhs_c[ii][0] + cocg[1] * rhs_c[ii][1] +
                         cocg[4] * rhs_c[ii][2]);
        grad_c[ii][2] = (cocg[5] * rhs_c[ii][0] + cocg[4] * rhs_c[ii][1] +
                         cocg[2] * rhs_c[ii][2]);
      }
                   ...
                   cs_real_3_t grad[])
{
  ...
  cs_gradient_type_by_imrgra(imrgra, &gradient_type, &halo_type);
                                ...
                                cs_real_t                     grad[][3])
{
  ...
  if (hyd_p_flag == 1) {
    if (cs_glob_mesh->halo != nullptr) {
      cs_halo_sync_var_strided(cs_glob_mesh->halo,
                               halo_type,
                               (cs_real_t *)f_ext,
                               3);
      if (cs_glob_mesh->have_rotation_perio)
        cs_halo_perio_sync_var_vect(cs_glob_mesh->halo,
                                    halo_type,
                                    (cs_real_t *)f_ext,
                                    3);
    }
                        ...
                        cs_real_t                   grad[3])
{
  ...
  for (cs_lnum_t i = s_id; i < e_id; i++) {
    ...
    if (bc_coeff_a != nullptr && bc_coeff_b != nullptr) { /* Known face BC's */
      ...
      cs_real_t pfac =
        (bc_coeff_a[f_id] + (bc_coeff_b[f_id] - 1.) * var[c_id]) * unddij;
  ...
  grad[0] = (a00 * rhsv[0] + a01 * rhsv[1] + a02 * rhsv[2]) * det_inv;
  grad[1] = (a01 * rhsv[0] + a11 * rhsv[1] + a12 * rhsv[2]) * det_inv;
  grad[2] = (a02 * rhsv[0] + a12 * rhsv[1] + a22 * rhsv[2]) * det_inv;
}
...
#pragma omp parallel for
      for (int t_id = 0; t_id < n_i_threads; t_id++) {
        for (cs_lnum_t f_id = i_group_index[(t_id * n_i_groups + g_id) * 2];
          ...
          for (cs_lnum_t i = 0; i < 3; i++) {
            c_poro_div_duq[ii][i] +=
              i_poro_duq_0[f_id] * i_f_face_normal[f_id][i];
            c_poro_div_duq[jj][i] -=
              i_poro_duq_1[f_id] * i_f_face_normal[f_id][i];
          }
        }
      }
    }
...
#pragma omp parallel for
    for (int t_id = 0; t_id < n_b_threads; t_id++) {
      for (cs_lnum_t f_id = b_group_index[t_id * 2];
        ...
        cs_real_t veli_dot_n =
          (1. - b_f_face_factor[f_id]) * cs_math_3_dot_product(vel_i, normal);
        ...
        for (cs_lnum_t i = 0; i < 3; i++)
          c_poro_div_duq[ii][i] += b_poro_duq[f_id] * b_f_face_normal[f_id][i];
      }
